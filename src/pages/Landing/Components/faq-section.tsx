...
const FaqSection = React.forwardRef<HTMLElement, FaqSectionProps>(
  ({ className, title, description, items, contactInfo, ...props }, ref) => {
    return (
      <section
        ref={ref}
        className={cn('py-8 md:py-12 lg:py-16 w-full flex justify-center', className)}
        {...props}
      >
        <div className='container w-full max-w-[90%] sm:max-w-[85%] md:max-w-3xl lg:max-w-4xl mx-auto px-4 sm:px-6'>
          ...
          >
            <h2 className='text-2xl sm:text-3xl md:text-4xl font-semibold mb-2 md:mb-3'>{title}</h2>
            {description && (
