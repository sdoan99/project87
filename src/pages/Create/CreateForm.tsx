...
export default function CreateForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 text-gray-100 max-w-2xl mx-auto bg-gray-800 p-8 rounded-lg shadow-xl">
      {error && (
      ...
      <div>
        <label htmlFor="name" className="block text-sm font-medium mb-2 text-gray-200">Strategy Name</label>
        <input
          id="name"
          type="text"
          value={name}
      ...
      <div>
        <label htmlFor="description" className="block text-sm font-medium mb-2 text-gray-200">Strategy Description</label>
        <textarea
          id="description"
          value={description}
        ...
        <div className="grid grid-cols-2 gap-4">
          {markets.map(market => (
            <div key={market} className="flex items-center space-x-2">
              ...
              />
              <label htmlFor={`market-${market}`} className="text-sm text-gray-300 hover:text-gray-100 cursor-pointer">
                {market}
              </label>
            </div>
          ))}
          ...
          {categories.map(category => (
            <div key={category} className="flex items-center space-x-2">
              <input
                ...
                className="w-4 h-4 text-blue-600 rounded border-gray-600 bg-gray-700 focus:ring-blue-500"
              />
              <label htmlFor={`category-${category}`} className="text-sm text-gray-300 hover:text-gray-100 cursor-pointer">
                {category}
              </label>
            </div>
          ))}
          ...
          {timeframes.map(timeframe => (
            <div key={timeframe} className="flex items-center space-x-2">
              <input
                ...
                className="w-4 h-4 text-blue-600 rounded border-gray-600 bg-gray-700 focus:ring-blue-500"
              />
              <label htmlFor={`timeframe-${timeframe}`} className="text-sm text-gray-300 hover:text-gray-100 cursor-pointer">
                {timeframe}
              </label>
            </div>
          ))}
